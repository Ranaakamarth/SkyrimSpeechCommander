...
namespace WindowsInput
{
    ...
    public static class InputSimulator
    {
        ...
        /// <summary>
        /// Retrieves scancode from virtualcode
        /// </summary>
        ...
        #region AsyncHold
        static System.Threading.Thread asyncHoldThread;

        static List<VirtualKeyCode> currentAsyncHoldKeys = new List<VirtualKeyCode>();
        ...
        public static void AsyncReleaseAllKeys()
        {
            lock (InputSimulator.currentAsyncHoldKeys)
            {
                foreach (VirtualKeyCode iter in InputSimulator.currentAsyncHoldKeys)
                {
                    InputSimulator.SimulateKeyUp(iter);
                }
                InputSimulator.currentAsyncHoldKeys.Clear();
            }
        }
        #endregion
        ...
        /// <summary>
        /// Syncronously simulates a keypress for a duration.
        /// </summary>
        /// <param name="keyCode">The VirtualkeyCode to press</param>
        /// <param name="heldDuration">The length of time (in milliseconds) to hold</param>
        public static void SimulateKeyPress(VirtualKeyCode keyCode, int heldDuration)
        {
            System.Diagnostics.Stopwatch watch = new System.Diagnostics.Stopwatch();
            watch.Start();
            while (watch.ElapsedMilliseconds < heldDuration)
            {
        ...
        public static void SimulateTextEntry(string text)
        {
            ...
            for (int x = 0; x < len; x++)
            {
                ...
                down.Type = (UInt32)InputType.KEYBOARD;
                down.Data.Keyboard = new KEYBDINPUT();
                down.Data.Keyboard.Vk = (ushort)MapVirtualKey(scanCode, 1);
                down.Data.Keyboard.Scan = scanCode;
                down.Data.Keyboard.Flags = 0;// (UInt32)KeyboardFlag.UNICODE;
                down.Data.Keyboard.Time = 0;
                down.Data.Keyboard.ExtraInfo = IntPtr.Zero;
                ...
                up.Type = (UInt32)InputType.KEYBOARD;
                up.Data.Keyboard = new KEYBDINPUT();
                up.Data.Keyboard.Vk = (ushort)MapVirtualKey(scanCode, 1);
        ...
        /// <summary>
        /// Syncronously simulates a modified keypress for a duration.
        /// </summary>
        ...
        public static void SimulateModifiedKeyStroke(VirtualKeyCode modifierKeyCode, VirtualKeyCode keyCode, int heldDuration)
        {
            ...
            while (watch.ElapsedMilliseconds < heldDuration)
            {
                WindowsInput.InputSimulator.SimulateKeyDown(modifierKeyCode);
                WindowsInput.InputSimulator.SimulateKeyDown(keyCode);
                System.Threading.Thread.Sleep(20);
            }
            watch.Stop();
            WindowsInput.InputSimulator.SimulateKeyUp(keyCode);
            WindowsInput.InputSimulator.SimulateKeyUp(modifierKeyCode);
        }
